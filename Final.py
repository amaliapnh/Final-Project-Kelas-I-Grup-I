...
def draw_bread(bread):
    window.blit(breadImg, (bread.x, bread.y))
        
#Window baker
def draw_window(baker):
    window.blit(backgroundImg, (0,0))
    window.blit(bakerImg, (baker.x, baker.y))
    
#Looping
def main():
    baker = pygame.Rect(230, 540, baker_width, baker_height)
    bread = pygame.Rect(260, 30, bread_width, bread_height)
    run = True
    while run:
        clock.tick(fps)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
               run = False   
        keys = pygame.key.get_pressed()
        if keys[pygame.K_LEFT] and baker.x - vel > 0:
            baker.x -= vel
        if keys[pygame.K_RIGHT] and baker.x + vel < 500:
            baker.x += vel          
        draw_window(baker)
        draw_bread(bread)
        pygame.display.update()
